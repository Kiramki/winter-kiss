...
label duke_check:
    ...
    if duke_encounters == 0:
        narrate "{i}The third and final location that stands out to you on your map is a noble estate in the capital, with a shield as its crest.{/i}"
        narrate "{i}You make your way over.{/i}"
    else:
    ...
    if duke_encounters == 0:
        narrate "{i}Coming up to the estate, you consider - are you ready to meet your final companion?{/i}"
    else:
    ...
    menu duke_menu:
        "Yes":
            jump duke_idle
        "No":
            narrate "{i}No, not yet. You decide to idle for a little while longer.{/i}"
...
label duke_encounter_tutorial:
    ...
    narrate "There are two knights on either side of the gate who wear the shield crest engraved in their pauldrons and on the badge attached to the front of their dark uniforms."
    narrate "They demand to know why you're there."
    ...
    menu duke_encounter_tutorial_guess:
        ...
        "Ask if he's The Duke":
            narrate "Without thinking, you blurt out the question. You watch as the harsh expression on the older gentleman relaxes - he almost looks amused."
            narrate "There's a kind twinkle in his eyes behind the glasses he wears, and he looks more like the grandfatherly figure that one would expect."
            narrate "He clears his throat. The amusement is gone, but his tone is a bit warmer when he repeats his question."
    ...
    narrate "You wait there for what must've been five or so minutes before the man returns and he allows the gates to be opened."
    narrate "Now that you've been allowed inside, you follow the man. You realize that you don't actually know who he is, but context clues suggest that he's not the master of this property."
    ...
    narrate "You think to introduce yourself again, but remember that you've already technically done so. You opt to stay silent instead."
    ...
    scene duke_main with Dissolve(1)
    pause 0.5
    show duke neutral with Dissolve(0.5)
    ...
    menu:
        ...
        "Claim your understanding of the prophecy":
            ...
            narrate "You explain to the Duke that the prophecy says the empire has a shield - and you believe that it's this household."
    ...
    scene duke_outside with Dissolve(0.5)
